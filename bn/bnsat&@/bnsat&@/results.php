...
<?php
	...
	$query = "SELECT COUNT(*) as num FROM $tableName WHERE status='N'";
    ...
    // Get page data
	$query1 = "SELECT * FROM $tableName WHERE status='N' ORDER BY income LIMIT $start, $limit";
...
<?php
echo '<form action="db1.php" method="POST">';
echo '<h3>Non updated Candidates:-</h3>';
echo '<br>';
...
<input type="radio" name="<?php echo $statuss.$ij ?>" value="W">Waitlist<br>
<input type="radio" name="<?php echo $statuss.$ij ?>" value="R">Reject<br>
<input type="radio" name="<?php echo $statuss.$ij ?>" value="N" checked>None</td>
<?php
echo "</tr>";
}
...
echo '<td colspan="1"><input type="button" value="Incomplete Applications" onclick="brown()"></td>';
echo '<td colspan="1"><input type="button" value="Reject All" onclick="unknown()"></td>';
echo "</tr>";
echo "</table";
